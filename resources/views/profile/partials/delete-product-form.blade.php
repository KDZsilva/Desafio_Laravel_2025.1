    ...
    <x-modal name="confirm-product-deletion{{$product->id}}" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('admin.products.destroy', $product->id) }}" class="p-6">
            ...
            <h2 class="text-lg font-medium text-gray-900 text-left">
                {{ __('Tem certeza que deseja deletar esse produto?') }}
            </h2>

            <p class="mt-1 text-sm text-gray-800 text-left">
                {{ __('Uma vez que um produto for deletado, ele e todos os seus dados relacionados vão ser permanentemente deletado. Por favor digite sua senha para confirmar que gostaria de deletar permanentemente esse produto.') }}
            </p>
